...
| 60〜35秒 | 欠けが進む | ティール → ローズへ徐々に補間 | 数字も同じ補間色 |
| 約35〜0秒 | 血月(ローズ)化が進み、柔らかいグローが差す(`t >= 0.5` でグロー付与) | ローズ(#f0949b) | — |
| 10〜0秒 | 血月化が深まる | ローズ(#f0949b) | 1秒周期の静かな鼓動パルス(scale 1.06、バウンス無し) |
| 0秒 | — | — | `onDone` 発火(= 現行 `limittimeDone`) |

- 色補間の基準:残り 60 秒から `t = clamp((60 - rem) / 50, 0, 1)`(rem>60 は 0、rem≤10 は 1)で teal→rose を線形補間。グロー(血月)は `t >= 0.5`(残り約35秒)で付与し、鼓動パルスは残り10秒以下で開始する。
